    ...
    def start(self):
        ...
        while begin_date <= end_date:
            ...
            for land in lands:
                ...
                self.output.table_name="world_landdata_" + begin_date.strftime("%Y_%m_%d")
                self.output.create()
                            ...
                            if(station_response!=None):
                                ...
                                if(station_list!=None):
                                    for station in station_list:
                                        ...
                                        data_result_list = []
                                        # 先查找本次循环日期的00-11点的数据
                                        data_response = self.download.data_download(station['StationID'],begin_date_str+" 00",begin_date_str+" 11",land,country)
                                        # 对00-11点得到的实时数据进行解析
                                        if(data_response!=None):
                                            data_result_list = self.parser.data_parser(data_response,station['CNAME'])
                                        # 再查找本次循环日期的12-23点的数据
                                        data_response = self.download.data_download(station['StationID'],begin_date_str+" 12",begin_date_str+" 23",land,country)
...
if __name__=="__main__":
    ...
    logging.basicConfig(level=logging.DEBUG,
                        ...
                        filename=r'world_land_log/' + datetime.now().strftime("%Y%m%d%H%M%S") + '.log',
                        filemode='w')
